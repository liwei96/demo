<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:76:"D:\php\PHPTutorial\WWW\tp\public/../application/admin\view\manager\edit.html";i:1557478161;s:70:"D:\php\PHPTutorial\WWW\tp\public/../application/admin\view\layout.html";i:1553218686;}*/ ?>
<!DOCTYPE html>
<html lang="en">
...
<div class="content">
    ...
    <div class="well">
        <!-- edit form -->
        <form action="<?php echo url('admin/manager/update',['id'=>$data['id']]); ?>" method="post" id="tab">
            <label>用户名：</label>
            <input type="text" name="username" value="<?php echo $data['username']; ?>" class="input-xlarge">
            <label>昵称：</label>
            <input type="text" name="nickname" value="<?php echo $data['nickname']; ?>" class="input-xlarge">
            <label>邮箱：</label>
            <input type="text" name="email" value="<?php echo $data['email']; ?>" class="input-xlarge">
            <label>角色：</label>
            <select name="role_id" id="">
                <?php foreach($roles as $v): ?>
                <option value="<?php echo $v['id']; ?>" <?php if($data['role_id']==$v['id']): ?>selected<?php endif; ?>><?php echo $v['role_name']; ?></option>
                <?php endforeach; ?>
            </select>
